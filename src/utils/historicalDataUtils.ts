import type { HistoricalLog, StoveData } from '../types';

/**
 * Flatten a Firebase HistoricalLog object into a chronologically sorted array
 * of StoveData entries.  Adds absolute timestamp (milliseconds) computed from
 ...
 * @returns Sorted array of StoveData objects, each including `id_timestamp` and
 *          `__historical` flag so that downstream code (preprocess, charts)
 *          can treat them correctly.
 */
export function flattenHistoricalLog(log: HistoricalLog, baseTimestamp: string | number): StoveData[] {
  ...
  const out: StoveData[] = Object.entries(log)
 ...
 */
export function mergeHistoricalLogs(logs: Array<{ log: HistoricalLog; baseTimestamp: string | number }>): StoveData[] {
  const merged: StoveData[] = [];
